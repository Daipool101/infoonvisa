...
// Outputs are written on every path and the script exits 0 unless something
// genuinely went wrong — see the same note in verdict-review.mjs. `checked` is
// reported even on a healthy run so the monthly proof-of-life can say
// "293 links checked, all healthy" rather than just "fine".
function emit(pairs) {
...
if (!broken.length) {
  ...
  emit({ checked: urls.size, broken: 0, body: '' });
  process.exit(0);
}
...
emit({ checked: urls.size, broken: broken.length, body });
process.exit(0);
